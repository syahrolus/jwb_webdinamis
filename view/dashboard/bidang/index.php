            ...
            <tbody>
                ...
                if ($query = mysqli_query($koneksi, $query)) {
                    while ($data = mysqli_fetch_array($query)) {
                        echo "
                        <tr>
                            <th scope='row'>" . $data["id_bidang"] . "</th>
                            <td>" . $data["nama_bidang"] . "</td>
                            <td>
                                <div class='d-flex'>
                                    <form action='edit_bidang.php' method='POST' style='margin-right: 5px;'>
                                        <input hidden type='text' name='id' value=" . $data["id_bidang"] . ">
                                        <button type='submit' name='edit' class='btn btn-primary'>Edit</button>
                                    </form>
                                    <a class='btn btn-danger' onclick='hapus_row_bidang(" . $data["id_bidang"] . ")' href='#' role='button'>Hapus</a>
                                </div>
                            </td>
                        </tr>";
                    }
